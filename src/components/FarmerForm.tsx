    ...
    try {
      ...
      const firstPageContent = `
        <div style="text-align: center; margin-bottom: 40px;">
          <h1 style="font-size: 24px; font-weight: bold; text-decoration: underline; margin: 0 0 50px 0;">ચકાસણી અહેવાલ</h1>
...
જિલ્લો-${getFieldValue(formData.district)} ને જિલ્લા ખેતીવાડી અધકારી, જિલ્લા પંચાયત, Sabarkatha દ્વારા તારીખ ${getFieldValue(formData.billDate)} થી પુવમંજુરી આપેલ છે જેની આજ રોજ ભૌિતક ચકાસણી કરતા સરકાર માન્ય ટ્રેક્ટર કંપની ${getFieldValue(formData.tractorCompany)} નું માન્ય મોડેલ ${getFieldValue(formData.modelCode)} ચેસીઝ  નંબર ${getFieldValue(formData.chassisNumber)} એન્જિન નંબર ${getFieldValue(formData.engineNumber)} જેનો RTO પાસિંગ નંબર ${getFieldValue(formData.rtoPassingNumber)} તારીખ ${getFieldValue(formData.rtoPassingDate)} થી ઓથોરાઈઝ ડીલર ${getFieldValue(formData.dealerName)} પાસેથી બિલ નંબર ${getFieldValue(formData.billNumber)} તારીખ ${getFieldValue(formData.billDate)} થી ખરીદ કરેલ છે. અને જેનાં રજુ થયેલ સાધનીક કાગળોની ચકાસણી કરતા બરાબર જણાય છે./જણાતું નથી. તો સહાયની રકમની ચુકવણી કરવા ભલામણ કરવામાં આવે છે. / આવતુ નથી.ભલામણ નથી તેવા કિસ્સામાં<strong> → ટુંકી નોંધ--</strong>
      ...
      const secondPageContent = `
        <div style="text-align: center; margin-bottom: 30px;">
          ...
          <p>ખેડૂત લાભાર્થી <strong>${getFieldValue(formData.farmerName)}</strong> ગામ <strong>${getFieldValue(formData.village)}</strong>, તાલુકો <strong>${getFieldValue(formData.taluka)}</strong>,જિલ્લો-<strong>${getFieldValue(formData.district)}</strong> દ્વારા ટ્રેક્ટર ખરીદ કરીને રજુ કરેલ સાધનીક</p>
          <p style="margin-bottom: 30px;">કાગળોની યાદી</p>
        </div>
      ...
      const fourthPageContent = `
        ...
        <div style="text-align: justify; line-height: 1.8; font-size: 14px; margin-bottom: 20px;">
          ...
          <p style="margin-bottom: 20px;">
            આથી હુ નીચે સહી કરનાર<strong>${getFieldValue(formData.farmerName)}</strong> સરનામું ગામ <strong>${getFieldValue(formData.village)}</strong>,તાલુકો:<strong>${getFieldValue(formData.taluka)}</strong>,જિલ્લો-<strong>${getFieldValue(formData.district)}</strong> આથી લખી આપું છું કે <strong>${getFieldValue(formData.tractorCompany)}</strong> કંપની નું <strong>${getFieldValue(formData.modelCode)}</strong> ટ્રેક્ટર ખરીદ કરેલ છે. જેનો આર.ટી.ઓ પાસિંગ નંબર <strong>${getFieldValue(formData.rtoPassingNumber)}</strong> છે.જેની ખરીદ કિંમત રૂ. <strong>${getFieldValue(formData.purchaseAmount)}</strong> (આંકડે રૂપિયા <strong>Three Lakh</strong>) છે. જેમાં ખેતીવાડી ખાતા દ્વારા મારી સબસીડીની રકમ રૂ.<strong>${getFieldValue(formData.subsidyAmount) || '200000'}</strong>/-આંકડે રૂપિયા <strong>2 Lakh Rs</strong> ખૂબ મને ચૂકવી આપવાનો છે મને મળવા બરાબર છે.</p>
        </div>

        <div style="margin-bottom: 20px;">
          <p style="margin-bottom: 5px;"><strong>૧. લાભાર્થીનું બેંક મુજબ નામ:-</strong>${getFieldValue(formData.beneficiaryBankName)}</p>
          <p style="margin-bottom: 5px;"><strong>૨. બેંક નું નામ અને શાખા :-</strong>${getFieldValue(formData.bankNameAndBranch)}</p>
          <p style="margin-bottom: 5px;"><strong>૩. એકાઉન્ટ નંબર:-</strong>${getFieldValue(formData.accountNumber)}</p>
          <p style="margin-bottom: 20px;"><strong>૪. આઈ.એફ.એસ.સી. કોડ :-</strong>${getFieldValue(formData.bankIFSC)}</p>
        </div>

        <div style="display: flex; justify-content: space-between; margin-bottom: 15px;width:100%;">
          <div style="width: 70%;">
            <p style="margin-bottom: 15px;"><strong>૧.</strong></p>
            <p style="margin-bottom: 5px;">સાક્ષીની સહી:${getFieldValue(formData.witness1Signature)}</p>
            <p style="margin-bottom: 5px;">સાક્ષીનુ નામ:${getFieldValue(formData.witness1Name)}</p>
            <p style="margin-bottom: 5px;">ગામ ${getFieldValue(formData.witness1Village)}, તાલુકો${getFieldValue(formData.witness1Taluka)},જિલ્લો-${getFieldValue(formData.witness1District)}</p>
          </div>
          <div style="width: 100px; text-align: center; border: 2px solid black; padding: 15px; margin-top: 10px; height: 120px;">
            <p style="margin-bottom: 10px;"><strong>ખેડૂત ની સહી</strong></p>
            <div style="height: 30px;"></div>
            <p style="font-size: 12px;"><strong>રેવḍયુટીકીટ લગાવવી</p>
          </div>
        </div>

        <div style="margin-bottom: 15px;">
          <p style="margin-bottom: 15px;"><strong>૨.</strong></p>
          <p style="margin-bottom: 5px;">સાક્ષીની સહી:${getFieldValue(formData.witness2Signature)}</p>
          <p style="margin-bottom: 5px;">સાક્ષીનુ નામ:${getFieldValue(formData.witness2Name)}</p>
          <p style="margin-bottom: 5px;">ગામ ${getFieldValue(formData.witness2Village)}, તાલુકો${getFieldValue(formData.witness2Taluka)},જિલ્લો-${getFieldValue(formData.witness2District)}</p>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="text-center mb-8 border-b-2 border-green-200 pb-6">
            <h1 className="text-3xl font-bold text-green-800 mb-2">ચકસણી અહેવાલ ફોર્મ</h1>
            <p className="text-gray-600">કૃષક માટે ચકાસણી અહેવાલ ફોર્મ</p>
          </div>
            ...
            <div className="bg-indigo-50 rounded-xl p-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    બેંક નું નામ અને શાખા
                  </label>
                  <input
                    {...register('bankNameAndBranch')}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
